    ...
    and not exists (
      ...
      where not (
        ...
        or (
          ...
          and exists (
            select 1
            from unnest(string_to_array(regexp_replace(coalesce(b.search_doc, ''), '[^a-z0-9]+', ' ', 'g'), ' ')) w
            where length(w) >= 3
              and extensions.levenshtein_less_equal(
                    w, toks.t, case when length(toks.t) <= 6 then 1 else 2 end
                  ) <= case when length(toks.t) <= 6 then 1 else 2 end
          )
        )
      )
    )
