...
DROP FUNCTION IF EXISTS public.admin_live_traffic();
DROP FUNCTION IF EXISTS public.admin_live_traffic(boolean);
...
BEGIN
  ...
  session_class AS (
    SELECT
      ...
      EXISTS(
        SELECT 1 FROM admin_ids a, sess_users su
         WHERE su.session_id = e.session_id
           AND a.user_id::text = su.user_id_text
      ) AS is_admin_user
    FROM ev_all e
  ...
  ev AS (
    SELECT e.*, k.kind, k.bot_name
    FROM ev_all e
    JOIN session_kind k USING (session_id)
    WHERE (NOT _exclude_admins OR k.kind <> 'admin')
  ),
  ev_human AS (
    SELECT * FROM ev WHERE kind = 'human'
  ),
  online_human AS (
    SELECT * FROM ev_human WHERE created_at >= _online_since
  ),
  sessions AS (
    SELECT
      session_id,
  ...
  intent AS (
    SELECT session_id, LEAST(100, s_oem_repeat + s_part_repeat + s_wa + s_call + s_fav)::int AS score
    FROM intent_raw
  ),
  sessions_enriched AS (
    SELECT
      s.session_id,
      s.joined_at, s.last_at, s.landing_path, s.path, s.city, s.country,
      ...
      END AS browser,
      CASE
        WHEN s.first_referrer IS NULL OR s.first_referrer = '' THEN 'Direkt'
  ...
  bots_window AS (
    SELECT
      COALESCE(k.bot_name, 'Diğer Bot') AS bot_name,
  ...
  sources_json AS (
    SELECT COALESCE(jsonb_agg(jsonb_build_object('source', source, 'count', cnt) ORDER BY cnt DESC), '[]'::jsonb) AS arr
  ...
  cities AS (
    SELECT
      ...
      END AS city,
      COUNT(*) AS cnt
    FROM sessions GROUP BY 1 ORDER BY cnt DESC LIMIT 30
  ),
  cities_json AS (
    SELECT COALESCE(jsonb_agg(jsonb_build_object('city', city, 'count', cnt) ORDER BY cnt DESC), '[]'::jsonb) AS arr FROM cities
  ),
  pages AS (
    SELECT
      CASE
  ...
  funnel_json AS (
    SELECT jsonb_build_object(
      ...
      'contacted', contacted, 'favorited', favorited
    ) AS obj FROM funnel
  ),
  hot_parts AS (
  ...
  stream AS (
    SELECT e.event_type, e.path, e.city, e.metadata, e.created_at,
           right(e.session_id, 6) AS session,
           k.kind, k.bot_name
    FROM ev e
    JOIN session_kind k USING (session_id)
  ...
  SELECT jsonb_build_object(
    ...
    'counts',      (SELECT jsonb_build_object('real_users', real_users, 'bots', bots, 'admins', admins) FROM counts),
