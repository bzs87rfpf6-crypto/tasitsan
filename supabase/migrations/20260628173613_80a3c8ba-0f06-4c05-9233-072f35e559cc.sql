...
BEGIN
  ...
  vis AS (
    SELECT
      COUNT(DISTINCT session_id) FILTER (WHERE session_id IS NOT NULL) AS total30,
      COUNT(DISTINCT session_id) FILTER (WHERE session_id IS NOT NULL AND created_at >= _day_start) AS today,
      COUNT(DISTINCT session_id) FILTER (WHERE session_id IS NOT NULL AND created_at >= _week_start) AS week,
      COUNT(DISTINCT session_id) FILTER (WHERE session_id IS NOT NULL AND created_at >= _month_start) AS month
    FROM pv
  ),
  session_geo AS (
    SELECT DISTINCT ON (session_id) session_id, city, country
    FROM pv
    WHERE session_id IS NOT NULL
    ORDER BY session_id, created_at ASC
  ),
  tr_cities AS (
    SELECT initcap(lower(city)) AS city, COUNT(*) AS cnt
    FROM session_geo
    WHERE (country ~* '^(turkey|türkiye|turkiye|tr)$' OR country IS NULL)
  ...
  abroad AS (
    ...
    WHERE country IS NOT NULL
      AND country !~* '^(turkey|türkiye|turkiye|tr)$'
  ),
  cities_list AS (
    SELECT jsonb_build_object('city', city, 'count', cnt) AS j, cnt
    FROM tr_cities
    UNION ALL
    SELECT jsonb_build_object('city', 'Yurtdışı', 'count', cnt), cnt
    FROM abroad WHERE cnt > 0
  ),
  cities_json AS (
  ...
  daily_series AS (
    SELECT
      dy.date,
      COALESCE((SELECT COUNT(DISTINCT session_id) FROM pv WHERE session_id IS NOT NULL AND created_at >= dy.day_start_ts AND created_at < dy.day_end_ts), 0) AS visitors,
  ...
  SELECT jsonb_build_object(
    ...
    'debug', jsonb_build_object(
      ...
      'distinctSessionsLast30', (SELECT total30 FROM vis),
