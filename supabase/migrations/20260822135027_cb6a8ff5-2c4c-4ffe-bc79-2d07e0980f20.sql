...
as $$
with toks as (
  select distinct lower(trim(t)) as tk
  from unnest(coalesce(_tokens, '{}'::text[])) t
  where length(trim(t)) >= 2
),
alts as (
  select toks.tk, a.a
  from toks, lateral unnest(string_to_array(toks.tk, '|')) a(a)
  where length(a.a) >= 2
),
qn as (
  ...
  where (select count(*) from toks) = 0
     or not exists (
       ...
       where not exists (
         ...
         where a.tk = toks.tk and coalesce(b.search_doc, '') like '%' || a.a || '%'
       )
     )
),
fuzzy_m as (
  ...
  where (select count(*) from toks) > 0
    ...
    and not exists (
      ...
      where not exists (
        ...
        where a.tk = toks.tk
          and (
            coalesce(b.search_doc, '') like '%' || a.a || '%'
            or (length(a.a) >= 4 and public.fuzzy_word_hit(b.search_doc, a.a))
          )
      )
    )
),
...
partial_m as (
  ...
  where (select count(*) from toks) >= 2
    ...
    and (
      select count(distinct a.tk) from alts a
      where coalesce(b.search_doc, '') like '%' || a.a || '%'
    ...
    (
      ...
      + coalesce((
          select sum(best) from (
            select max(
              case
                when lower(coalesce(p.title, '')) like '%' || a.a || '%' then 60
                when lower(coalesce(p.brand, '') || ' ' || coalesce(p.model, '')) like '%' || a.a || '%' then 40
                when coalesce(p.search_doc, '') like '%' || a.a || '%' then 25
                else 12
              end
            ) as best
            from alts a
