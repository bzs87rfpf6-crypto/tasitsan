...
export const adminSetRole = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertOwnerAdmin(context.supabase, context.userId);
    void data;
    throw new Error("Yönetici hesabı sabittir ve devredilemez");
  });
...
export const adminSetTrustedSeller = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d) =>
    z.object({ userId: z.string().uuid(), trusted: z.boolean() }).parse(d),
  )
  .handler(async ({ data, context }) => {
    await assertOwnerAdmin(context.supabase, context.userId);
    // Güncelleme, çağıran yöneticinin kimliğiyle (auth.uid) çalışan güvenli RPC
    // üzerinden yapılır; böylece trusted_seller trigger'ı doğru şekilde geçer.
    const { error } = await context.supabase.rpc("admin_set_trusted_seller" as never, {
      _user_id: data.userId,
      _trusted: data.trusted,
    } as never);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
