...
import { LogOut, Package, Pencil, Power, Trash2, Heart, ClipboardList, Bell, Flame, KeyRound, TrendingUp, Clock, CheckCircle2 } from "lucide-react";
...
function AccountPage() {
  ...
  const sellerStats = useMemo(() => {
    ...
    const stockValue = myParts
      .filter((p) => p.status === "approved")
      .reduce((sum, p) => sum + (Number(p.price) || 0) * Math.max(0, p.stock_quantity ?? 1), 0);
    return { total, active, pending, stockValue };
  }, [myParts]);
    ...
    <div className="min-h-screen pb-24">
      ...
      <div className="max-w-md mx-auto px-4 pt-4 space-y-6">
        ...
        <section className="grid grid-cols-2 gap-2.5">
          <div className="bg-card border border-border rounded-xl p-3">
            <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-muted-foreground">
              <Package className="size-3.5 text-gold" /> Toplam Ürün
            </div>
            <p className="mt-1 font-display text-2xl text-foreground">{sellerStats.total}</p>
          </div>
          <div className="bg-card border border-border rounded-xl p-3">
            <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-muted-foreground">
              <CheckCircle2 className="size-3.5 text-emerald-400" /> Aktif Ürün
            </div>
            <p className="mt-1 font-display text-2xl text-emerald-400">{sellerStats.active}</p>
          </div>
          <div className="bg-card border border-border rounded-xl p-3">
            <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-muted-foreground">
              <Clock className="size-3.5 text-amber-400" /> Bekleyen
            </div>
            <p className="mt-1 font-display text-2xl text-amber-400">{sellerStats.pending}</p>
          </div>
          <div className="bg-card border border-border rounded-xl p-3">
            <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-muted-foreground">
              <TrendingUp className="size-3.5 text-gold" /> Stok Değeri
            </div>
            <p className="mt-1 font-display text-lg text-gold">
              ₺{sellerStats.stockValue.toLocaleString("tr-TR")}
            </p>
            <p className="text-[10px] text-muted-foreground">yayındaki ürünler</p>
          </div>
        </section>
        ...
        <Link
          to="/sell/bulk"
          search={{ mode: "price" }}
          className="block bg-card border border-gold/40 rounded-xl p-4 hover:bg-gold/5 transition no-underline"
        >
          <p className="text-sm font-semibold text-gold">Toplu Fiyat Güncelle</p>
          <p className="text-[11px] text-muted-foreground mt-0.5">
            Excel ile OEM numarasına göre fiyatlarınızı toplu güncelleyin.
          </p>
        </Link>
