...
import { findSellerDuplicateByOem } from "@/lib/seller-duplicates";
...
function SellPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const duplicate = await findSellerDuplicateByOem(user.id, oemCodes);
    if (duplicate) {
      toast.error(`Bu OEM ile zaten bir ilanınız var: "${duplicate.title}". Mevcut ilanı düzenleyin.`);
      return;
    }
