...
function DashboardPanel({
  ...
  const inactive = parts.filter((p) => (p.status as string) === "inactive").length;
  const rejected = parts.filter((p) => p.status === "rejected").length;
  const statusSum = pending + approved + inactive + rejected;
  ...
  return (
    <div className="space-y-4">
      <p className="text-[10px] text-muted-foreground text-center">
        Canlı veri · {parts.length} ilan ({approved} yayında, {pending} bekliyor)
        {statusSum === parts.length ? " ✓" : ` · uyumsuzluk: ${parts.length - statusSum}`}
      </p>
      <div className="grid grid-cols-2 gap-2.5">
        ...
        <button onClick={() => onJump("products")} className="text-left">
          <StatCard icon={<Package className="size-3.5" />} label="Yayındaki İlan" value={approved} accent="text-emerald-400" />
        </button>
