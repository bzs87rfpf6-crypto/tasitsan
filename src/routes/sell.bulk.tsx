...
type Mode = "insert" | "update" | "stock" | "price";
...
function parseRows(raw: Record<string, unknown>[], mode: Mode): Row[] {
  return raw.map((r, i) => {
    ...
    if (mode === "price") {
      ...
      if (qty < 0) errors.push("ADET geçersiz");
    } else {
...
function BulkUploadPage() {
  ...
  useEffect(() => {
    ...
    if (urlMode === "price" || urlMode === "stock" || urlMode === "update" || urlMode === "insert") {
      setMode(urlMode);
    }
    if (result.ok) {
  ...
  const downloadTemplate = () => {
    if (mode === "price") {
      ...
      a.href = url;
      a.download = "tasitsan-toplu-fiyat-sablonu.xlsx";
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      toast.success("Fiyat şablonu indirildi");
      return;
    }
  ...
  const submit = async () => {
    ...
    const rowsToProcess =
      mode === "insert" ? valid.filter((r) => !r.duplicate) : valid;
    const skippedDuplicates = mode === "insert" ? valid.filter((r) => r.duplicate).length : 0;

    if (rowsToProcess.length === 0) {
      if (skippedDuplicates > 0) {
        toast.error(`${skippedDuplicates} satır mükerrer OEM içeriyor. Mevcut ilanlarınızı güncelleyin.`);
      } else {
    ...
    if (skippedDuplicates > 0) {
      toast.warning(`${skippedDuplicates} mükerrer OEM satırı atlandı.`);
    }
    ...
    <div className="min-h-screen pb-28">
      ...
      <div className="max-w-2xl mx-auto px-4 pt-4 space-y-4">
        ...
        <div className="rounded-xl border border-gold/30 bg-gold/5 px-3 py-2.5 text-[11px] text-muted-foreground leading-relaxed">
          ...
          {mode === "price" && <><span className="text-gold font-semibold">Fiyat Güncelle:</span> Yalnızca <em>OEM NO</em> ve <em>FİYAT</em> sütunları kullanılır; mevcut ilanlarınızın fiyatı güncellenir.</>}
        ...
        {rows.length > 0 && !result && (
          ...
          >
            {submitting ? "İşleniyor..." : `${valid.length} kaydı ${
              mode === "insert" ? "onaya gönder" : mode === "price" ? "fiyat güncelle" : "güncelle"
            }`}
          </Button>
        )}
